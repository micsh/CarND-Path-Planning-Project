		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					// Previous path data given to the Driver
					...
					car.update(car_s, car_d, car_speed);

					vector<vector<Car>> carsOnRoad;
					carsOnRoad.resize(road.Lanes());
					...
					for (int i = 0; i < sensor_fusion.size(); ++i) {
						...
						if (d >= 0 && d <= road.Width())
						{
							Car carOnRoad(id, s, d, v);
							carsOnRoad[road.lane(carOnRoad)].push_back(carOnRoad);
						}
					}
